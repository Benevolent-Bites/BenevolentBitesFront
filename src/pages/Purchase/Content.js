...
class PurchaseContent extends React.Component {
  ...
  async componentDidMount() {
    ...
    try {
      this.setState({
        data: await this.checkBackend(restGetDetails() + "?restId=" + restId),
      });
    } catch (err) {
      console.log(err);
      this.setState({
        data: { error: "Sorry, could not load restaurant info" },
      });
    }
  }
        ...
        {Object.keys(data).length === 0 ? (
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12}>
              <GridList cols={3}>
                {data.customPhotos &&
                  data.customPhotos.map((url) => <img alt="team" src={url} />)}
              </GridList>
              {data.hasOwnProperty("customPhotos") &&
                data.customPhotos === null &&
                data.image && (
                  <img
                    alt="restaurant"
                    src={
            ...
            <Grid item xs={12}>
              <Typography variant="body2" style={{ fontSize: "1rem" }}>
                Important: By purchasing a gift card to{" "}
                <strong>{data.name}</strong>, you are donating
                <strong> 25% of your purchase </strong> to help restaurant staff
                survive the worst economic circumstances ever to hit the service
                industry. The remaining 75% will be available for you to redeem
                at {data.name}.
              </Typography>
            </Grid>
          </Grid>
