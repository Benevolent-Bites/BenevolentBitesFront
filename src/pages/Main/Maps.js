...
class GoogleMapsContainer extends React.Component {
  ...
  render() {
    ...
    return (
      ...
      >
        {this.props.list.map((place) => <Marker
            position={{ lat: place.latitude, lng: place.longitude }}
            name={place.title}
            title={place.title}
            onClick={this.onMarkerClick}
            place={place}
            icon={place.on ? null : {url: process.env.PUBLIC_URL + "/img/greymarker.png"}}
          />
        )}
        <InfoWindowEx
        ...
        >
          {Object.keys(this.state.place).length > 0 && <RestaurantCard map signedIn={this.props.signedIn}
            data={this.state.place} supported={this.state.place.on} classes={this.props.classes}/>}
        </InfoWindowEx>
      </Map>
    );
  }
